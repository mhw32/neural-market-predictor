...
% https://urldefense.proofpoint.com/v2/url?u=http-3A__www.cis.hut.fi_projects_somtoolbox_&d=AwIGAg&c=-dg2m7zWuuDZ0MUcV7Sdqw&r=W1P4bb8ZQsAct4qSkcVXpw&m=McgzQuraJZH19RmM5W2Mowgi5DhJEk8chiuHt1LLpVg&s=BMTM83yAruwd4lYOcZbgrSnQKeRV_veFK-_3mmpi8fM&e=              
...
% 		     'LabelSize', Property.textsize);
  ...
  dim = size(Labels,1);
  positive_indices = zeros(dim, 1);
  negative_indices = zeros(dim, 1);
  
  for counter = 1:dim
      next = Labels{counter};
      if ~isempty(next)
          if next == 1
              positive_indices(counter) = 1;
          elseif next == 0
              negative_indices(counter) = 1;
          else
              display('ERROR!!!!!!');
          end
      end
  ...
  positive_labels = cell(dim, 1);
  for counter = 1 : dim
      if positive_indices(counter) == 1
          positive_labels{counter} = 1;
      end
  end
  
  negative_labels = cell(dim, 1);
  for counter = 1 : dim
      if negative_indices(counter) == 1
          negative_labels{counter} = 1;
      end
  end
