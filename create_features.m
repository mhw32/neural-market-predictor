function features_data = create_features(hist_fund_data, stock_data, market_data)   
   ...
   for k=1:length(tickers)
       ...
       for i=1:length(existing_features_data)
           % Converting to a double makes all the empty slots --> NaN perfect!
           valuez = [valuez cellfun(@(x)str2double(x), {existing_features_data(:, i, 1)}, 'UniformOutput', false)];
       end
       ...
       new_features_data = {fs.get_shares_outstanding(all_f), fs.get_current_ratio(all_f), fs.get_curr_over_tot_assets(all_f), ... 
           ...
           fs.get_stock_adj_close_price(stock_data.(dummy_var)), fs.get_log_revenue_return(market_data)}; 
